...
def Consistent_Check(Lat, Out, inv):
  if inv==(INVOC-1):
    string_A = KEC.lane2hex(Out)
    return Lat==string_A[:(D_Size//4)]
  else:
    stateA = np.array(Lat[(R_Size//64):], dtype=np.uint64)
    stateB = np.array(Out[(R_Size//64):], dtype=np.uint64)
...
class Recovery:
  def __init__(self):
    print('Loading input (answer) strings...')
    self.ans_inputs = []
    ...
    print('Loading output (verifying) strings...')
    self.ver_outputs = []
    for t in range(SET_L, SET_U):
      ver_name = 'data_raw_out/set_I'+str(INVOC).zfill(2)+'_'+str(t).zfill(4)+'_outputs.npy'
      string_outputs = svm.Load(ver_name)
      for s in range(0, len(string_outputs)):
        self.ver_outputs.append(string_outputs[s])
  ...
  def recover(self, tr):
    ...
    tS = time.time()
    rec_IN = ''
    latter_state = self.ver_outputs[tr]
    Caps = [-1]*INVOC
    Suc_state = True
    for invoc in list(range(0, INVOC))[::-1]:
      ...
      while Capacity<2560000:
        ...
        if Consistent_Check(latter_state, inv_out, invoc):
          print('Consistence check passed!')
          if invoc!=(INVOC-1):
            IN_XOR_bin = []
            for lane in range(0, 25):
              IN_XOR_bin.append(inv_out[lane]^latter_state[lane])
            IN_XOR_hex = KEC.lane2hex(IN_XOR_bin)
            rec_IN = (IN_XOR_hex[:(R_Size//4)])+rec_IN
          latter_state = inv_in[:]
          if invoc==0:
            rec_IN = KEC.lane2hex(inv_in)[:(R_Size//4)]+rec_IN
          break
        Capacity *= 4
      Caps[invoc] = Capacity
      if Capacity==2560000:
