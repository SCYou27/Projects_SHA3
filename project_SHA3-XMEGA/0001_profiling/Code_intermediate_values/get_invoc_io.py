...
def find_IO():
  ...
  short_input = []
  short_output = []
  for set_n in range(0, 200):
    print("Loading files of set #"+str(set_n))
    infile = np.load(("data_raw_in/set_TR_"+str(set_n).zfill(4)+"_inputs.npy"))
    outfile = np.load(("data_raw_out/set_TR_"+str(set_n).zfill(4)+"_outputs.npy"))
    for t in range(0, 40):
